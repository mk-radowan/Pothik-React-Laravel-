                                    ...
                                    <td style="min-width:220px">
                                        @if (($booking->payment_status ?? 'unpaid') === 'paid')
                                            <span class="badge" style="background:#dcfce7;color:#166534;border:1px solid #bbf7d0">
                                                Paid via {{ strtoupper($booking->payment_method ?? '-') }}
                                            </span>
                                            <div style="font-size:0.75rem;color:#64748b;margin-top:4px">
                                                Ref: {{ $booking->payment_reference ?? 'N/A' }}
                                            </div>
                                            <div class="mt-2 d-flex gap-1 flex-wrap">
                                                <a href="{{ route('bookings.invoice', $booking->id) }}" class="btn btn-sm btn-outline-secondary">
                                                    View Invoice
                                                </a>
                                                <a href="{{ route('bookings.invoice.download', $booking->id) }}" class="btn btn-sm btn-outline-red">
                                                    Download
                                                </a>
                                            </div>
                                        @elseif(($booking->payment_status ?? 'unpaid') === 'processing')
                                            <span class="badge" style="background:#e0f2fe;color:#075985;border:1px solid #bae6fd">
                                                Payment Processing
                                            </span>
                                        @elseif(in_array($booking->payment_status ?? 'unpaid', ['failed', 'cancelled'], true))
                                            <span class="badge" style="background:#fee2e2;color:#991b1b;border:1px solid #fecaca">
                                                {{ ucfirst($booking->payment_status) }}
                                            </span>
                                            <div class="mt-1">
                                                <a href="{{ route('bookings.payment.show', $booking->id) }}" class="btn btn-sm btn-outline-red">
                                                    Retry Payment
                                                </a>
                                            </div>
                                        @else
